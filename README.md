...
- `public/js/` — vanilla ES-module client: `state` (identity + state bag), `net` (socket + reconnect), `audio`, `ui` (screens/toast), `home`, `identity-ui` (account card: link/recovery/claim), `lobby`, `game-view` (grid/fuse/caller panel), `sabotage-fx` (searcher-side sabotage visuals), `board-themes` (board picker motifs, announce splash, Blackout torch, Drift motion), `tournament-view`, `results`, `voice` (WebRTC mesh), `voice-meter` (WebAudio speaking detection + highlights), `install` (PWA prompt), `wake-lock` (screen stays on while in a room), `handlers` (server-message dispatch), `main` (wiring + boot). Design tokens are oklch CSS custom properties from the design system.
