...
class Room {
  ...
  broadcastVoice() {
    ...
    this.players.forEach((p, seat) => {
      const mine = this.voiceGroupOf(p.id);
      const peers = members
        .filter(m => m.id !== p.id && this.voiceGroupOf(m.id) === mine)
        .map(m => m.id);
  ...
  relayRtc(fromId, to, data) {
    ...
    if (this.voiceGroupOf(fromId) !== this.voiceGroupOf(to)) return;
  ...
  endTournament() {
    ...
    this.players.forEach(p => { if (p.active === false || !p.connected) this.voice.delete(p.id); });
