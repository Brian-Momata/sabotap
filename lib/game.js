...
class Room {
  constructor(code, onDestroy) {
    ...
    this.settings = { roundsToWin: CONFIG.roundsToWin, fuseMs: CONFIG.fuseMs };
  ...
  publicPlayers() {
    return this.players.map((p, seat) => ({ seat, id: p.id, name: p.name, tag: p.tag, connected: p.connected }));
  }
  ...
  addPlayer(player) {
    ...
    this.players.push({ ...player, connected: true, wantsRematch: false });
    this.broadcastRoom();
    return { ok: true, seat: this.players.length - 1 };
  }
  ...
  setSettings(seat, patch) {
    ...
    if (CONFIG.fuseMsOptions.includes(patch.fuseMs)) this.settings.fuseMs = patch.fuseMs;
    this.broadcastRoom();
  }
  ...
  goLive(target, wasRandom) {
    ...
    const base = {
      t: 'live',
      target,
      fuseMs: this.settings.fuseMs,
      puzzleTimeMs: CONFIG.puzzleTimeMs,
  ...
  startFuse() {
    ...
    this.timers.fuse = setInterval(() => {
      ...
      this.fuse = Math.min(1, this.fuse + CONFIG.fuseTickMs / this.settings.fuseMs);
  ...
  newPuzzle() {
    ...
    let odd;
    if (this.fuse > 0.4 && CONFUSABLE[shared].length) {
    ...
    this.send(this.callerSeat(), { t: 'puzzle', id: this.puzzle.id, tiles, timeMs: CONFIG.puzzleTimeMs });
