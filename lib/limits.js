...
class EdgeGuards {
  ...
  // otherwise a client could spoof its way past per-IP buckets. Take the LAST
  // entry: the proxy appends the real client IP to whatever XFF the client
  // sent, so every earlier entry is attacker-controlled — reading the first
  // would let anyone mint fresh budgets per connection.
  ipOf(req) {
