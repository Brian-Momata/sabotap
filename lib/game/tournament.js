...
class Tournament {
  constructor(room) {
    ...
    this.pendingPairs = null; // pairs announced but not yet started (splash showing)
  }
  ...
  nextStage() {
    ...
    this.announcePairs(toStart);
  }

  // Announce pairings, then start the matches after the splash delay. Also the
  // revival re-entry point when a restart cut the splash short.
  announcePairs(toStart) {
    this.pendingStagePairs = toStart.length;
    this.pendingPairs = toStart;
    ...
    this.pairingTimer = setTimeout(() => {
      ...
      // A player can vanish during the splash — hold that match until they return.
      this.stageMatches.forEach(({ match, pair }) => {
        if (!pair.every(s => this.players[s].connected)) match.pause();
      });
  ...
  // Nudge a revived schedule forward: replay a splash a restart cut short, or
  // re-arm the between-stage advance (live matches resume via reconnect).
  kick() {
    if (this.pendingPairs) return this.announcePairs(this.pendingPairs);
    this.checkStageDone();
  }
  ...
  createMatch(a, b) {
    return new Match({
      ...
      // Rotation marches with the stage — every pairing in a stage plays the
      // same boards (fair), and later stages reach the later boards.
      boardOffset: (this.stage - 1) * CONFIG.tournament.matchRounds,
      ...
      hooks: {
        ...
        onEnd: match => {
          telemetry.tournamentMatchEnded(match);
  ...
  endMessage() {
    ...
    telemetry.tournamentEnded(this);
