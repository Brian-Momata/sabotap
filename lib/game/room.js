...
const { makeVersusMatch } = require('./versus');
const { makeSoloMatch, soloSettings } = require('./solo');
const grace = require('./reconnect');
const telemetry = require('./telemetry');

/*
 * Room: roster, lobby settings, transport, and mode dispatch. A room runs one
 * versus match, one solo run against the bot (solo.js), or a Tournament (which
 * owns its own schedule and concurrent matches); voice signaling is delegated
 * to VoiceChannel, and disconnect grace/resume lives in reconnect.js. State
 * fields persisted across server restarts are mirrored in serialize.js —
 * extend it when adding room/match state.
 */
class Room {
  constructor(code, onDestroy) {
    ...
    this.settings = { mode: 'versus', roundsToWin: CONFIG.roundsToWin, difficulty: CONFIG.defaultDifficulty, board: CONFIG.defaultBoard };
    this.phase = 'lobby'; // lobby | playing | matchEnd | tEnd
    this.onPhaseChange = () => {}; // injected by the transport (friend presence refresh)
    this.log = [];
    this.boardCycle = 0;    // rotation rounds played; the next match resumes here
  ...
  maxPlayers() {
    if (this.isTournament()) return CONFIG.tournament.maxPlayers;
    return this.isSolo() ? 1 : 2;
  }
  minPlayers() {
    if (this.isTournament()) return CONFIG.tournament.minPlayers;
    return this.isSolo() ? 1 : 2;
  }

  // Solo is chosen when the room is created, never switched into later: the
  // seat cap is what keeps the run private, and flipping it under a seated
  // guest would strand them.
  setSolo() { this.settings = soloSettings(); }
  ...
  voiceJoin(seat, muted) { this.voice.join(this.players[seat], muted); }
  ...
  removePlayer(playerId) {
    ...
    // A departure during the final inter-round gap must not erase a decided
    // match — publish the result (room lands on matchEnd), then process the
    // leave as a post-match one.
    if (this.phase === 'playing' && this.match && this.match.decided()) {
      this.match.finishNow();
    }
  ...
  handleDisconnect(playerId) { grace.handleDisconnect(this, playerId); }
  reconnect(playerId, ws) { return grace.reconnect(this, playerId, ws); }
  sendResume(seat) { grace.sendResume(this, seat); }
  armGrace(playerId, ms) { grace.armGrace(this, playerId, ms); }
  ...
  setSettings(seat, patch) {
    ...
    if (CONFIG.boards[patch.board]) this.settings.board = patch.board;
    if (!this.isSolo() && (patch.mode === 'versus' || patch.mode === 'tournament')) {
  ...
  start(seat) {
    ...
    // A seat in reconnect grace (results-screen drop, revived room) would play
    // against dead air — hold the start until they return or expire.
    const away = this.players.find(p => !p.connected);
    if (away) {
      return this.sendTo(seat, { t: 'error', msg: `${away.name} is reconnecting. Give them a moment.` });
    }
  ...
  // ---------- match construction (one module per mode) ----------

  makeVersusMatch() { return makeVersusMatch(this); }
  makeSoloMatch() { return makeSoloMatch(this); }

  startVersus() {
    telemetry.matchStarted(this);
    this.match = this.makeVersusMatch();
    this.match.start();
  }

  startSolo() {
    telemetry.matchStarted(this);
    this.match = this.makeSoloMatch();
    this.match.start();
  }

  rematch(seat) {
    if (this.phase !== 'matchEnd') return;
    // Nobody to agree with: a solo run restarts on the spot.
    if (this.isSolo()) {
      this.resetScores();
      this.phase = 'playing';
      this.onPhaseChange();
  ...
  matchOf(seat) {
    if (!this.isTournament()) {
      if (this.match && this.match.phase !== 'done' && (seat === 0 || seat === 1)) {
        // `pair` is the occupied seats, not the match's seats: solo's second
        // seat is the bot, which has no roster entry to check for liveness or
        // relay opponent status to.
        return { match: this.match, pair: this.isSolo() ? [0] : [0, 1], seatIn: s => s };
      }
      return null;
    }
