...
class VoiceChannel {
  ...
  // muted lets a client join listen-only in one message, with no unmuted
  // flash in the roster between join and a follow-up mute.
  join(player, muted) {
    if (!player) return;
    // Reconnects re-join an existing member; only a genuinely new one counts.
    if (!this.members.has(player.id)) telemetry.voiceJoined(!!muted);
    this.members.set(player.id, { muted: !!muted });
    this.broadcastState();
  }
